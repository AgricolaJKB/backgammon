        ...
        .leftJoin(
            ...
            or(
                and(
                    ...
                    eq(user.id, friendships.receiverId),
                ),
                and(
                    eq(friendships.receiverId, userId),
                    eq(user.id, friendships.senderId),
                ),
            ),
        )
        .where(
...
export const actions = {
    ...
    createGame: async (event) => {
                ...
                and(
                    or(
                        and(
                            ...
                            eq(games.blackPlayerId, opponentId),
                        ),
                        and(
        ...
        if (existingGame.length > 0) {
            return fail(400, {
                message: 'Es läuft bereits ein Spiel gegen diesen Gegner',
            });
        }
    ...
    addFriend: async (event) => {
        ...
        if (existingFriendship) {
            ...
            if (existingFriendship.status === 'pending') {
                return fail(400, {
                    message: 'Freundschaftsanfrage bereits ausstehend',
                });
            }
        }
